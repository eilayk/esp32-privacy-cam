...
extern "C" void* create_face_detection_model()
{
    HumanFaceDetect *detect = new HumanFaceDetect();
    return detect;
}

extern "C" void destroy_face_detection_model(void *model)
{
    if (model != nullptr)
    {
        HumanFaceDetect *detect = static_cast<HumanFaceDetect *>(model);
        delete detect;
    }
}

extern "C" esp_err_t face_detection(void *model, const esp_dl_image_t *input_image, esp_dl_detection_list_t *out_result)
{
    if (model == nullptr || input_image == nullptr || out_result == nullptr || input_image->data == nullptr)
    {
        return ESP_ERR_INVALID_ARG;
...
extern "C" esp_err_t face_detection_annotate_jpeg(void *model,
                                                     const uint8_t *jpeg_data,
                                                     size_t jpeg_len,
                                                     esp_dl_detection_list_t *out_result,
                                                     esp_dl_jpeg_t *out_jpeg)
{
    ...
    esp_err_t err = face_detection(model, &raw_image, out_result);
    if (err == ESP_OK)
    {
        esp_dl_draw_detections(&raw_image, out_result);
        err = esp_dl_encode_jpeg(&raw_image, out_jpeg);
